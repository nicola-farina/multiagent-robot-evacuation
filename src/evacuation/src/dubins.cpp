...
namespace dubins {
    ...
    bool Dubins::areCurvesValid(CurveSegmentsResult *curve_segments, double k0, double k1, double k2, double th0, double thf) {
        ...
        double eq1 = x0 + curve_segments->s1 * sinc((1 / 2.) * k0 * curve_segments->s1) * cos(th0 + (1 / 2.) * k0 * curve_segments->s1) +
                     curve_segments->s2 * sinc((1 / 2.) * k1 * curve_segments->s2) * cos(th0 + k0 * curve_segments->s1 + (1 / 2.) * k1 * curve_segments->s2) +
                     curve_segments->s3 * sinc((1 / 2.) * k2 * curve_segments->s3) * cos(th0 + k0 * curve_segments->s1 + k1 * curve_segments->s2 + (1 / 2.) * k2 * curve_segments->s3) - xf;
        double eq2 = y0 + curve_segments->s1 * sinc((1 / 2.) * k0 * curve_segments->s1) * sin(th0 + (1 / 2.) * k0 * curve_segments->s1) +
                     curve_segments->s2 * sinc((1 / 2.) * k1 * curve_segments->s2) * sin(th0 + k0 * curve_segments->s1 + (1 / 2.) * k1 * curve_segments->s2) +
                     curve_segments->s3 * sinc((1 / 2.) * k2 * curve_segments->s3) * sin(th0 + k0 * curve_segments->s1 + k1 * curve_segments->s2 + (1 / 2.) * k2 * curve_segments->s3) - yf;
    ...
    bool Dubins::lineLineIntersection(DubinsPoint p1, DubinsPoint p2, DubinsPoint p3, DubinsPoint p4, std::vector<DubinsPoint> &pts, std::vector<double> &ts) {
        ...
        for (double t: ts) {
            pts.emplace_back((t * r.x) + p.x, (t * r.y) + p.y);
        }
    ...
    bool Dubins::arcLineIntersection(Arc *arc, DubinsPoint point1, DubinsPoint point2, std::vector<DubinsPoint> &pts, std::vector<double> &t) {
        ...
        if (std::abs(tanFirstAngle) <= 0 + EPSILON)
            m1 = INFINITY;
        if (std::abs(tanSecondAngle) <= 0 + EPSILON)
            m2 = INFINITY;

        DubinsPoint circleCenter = DubinsPoint(-1, -1);

        // Limit case: the slope is the same
        if (std::abs(m1 - m2) < 1.e-1 || std::abs(m1 + m2) < 1.e-1) {
        ...
        if (t1 >= 0 && t1 <= 1) {
            intersections.emplace_back(DubinsPoint((point1.x * t1) + (point2.x * (1 - t1)), (point1.y * t1) + (point2.y * (1 - t1))), t1);
        }

        if (t2 >= 0 && t2 <= 1 && t2 != t1) {
            intersections.emplace_back(DubinsPoint((point1.x * t2) + (point2.x * (1 - t2)), (point1.y * t2) + (point2.y * (1 - t2))), t2);
        }

        // Sort the intersections using t values
        std::sort(intersections.begin(), intersections.end(), [](const std::pair<DubinsPoint, double> a, const std::pair<DubinsPoint, double> b) { return a.second < b.second; });
        ...
        for (auto &intersection: intersections) {
            ...
            double intersectionTh = (atan2(intersection.first.y - circleCenter.y, intersection.first.x - circleCenter.x));
            ...
            if (orientation == 1) {
                ...
                if (firstTh < secondTh && (intersectionTh >= firstTh && intersectionTh <= secondTh)) {
                    pts.push_back(intersection.first);
                    t.push_back(intersection.second);
    ...
    double *Dubins::getAnglesShortestPath(DubinsPoint **points, int numberOfPoints, const std::vector<Polygon> &obstacles, const Polygon &map) {
        ...
        auto **L = new double *[numberOfPoints];
        ...
        for (unsigned int i = 0; i < K; i++) {
            Curve *curve = ShortestPathWithoutCollisions(points[numberOfPoints - 2]->x, points[numberOfPoints - 2]->y, multipointAngles[i], points[numberOfPoints - 1]->x, points[numberOfPoints - 1]->y, points[numberOfPoints - 1]->th,
                                                         obstacles, map);
            if (curve != nullptr) {
                isThereAPath = true;
    ...
    std::vector<std::vector<evacuation::Point>> Dubins::getObstaclesAndMapLines(std::vector<evacuation::Polygon> obstacles, evacuation::Polygon map) {
        std::vector<std::vector<Point>> obstaclesLines;
        for (auto &obstacle: obstacles) {
            for (const std::vector<Point> &edge: obstacle.getPolygonEdges()) {
                obstaclesLines.push_back(edge);
            }
        }
        for (const std::vector<Point> &edge: map.getPolygonEdges()) {
            obstaclesLines.push_back(edge);
        }
    ...
    Curve *Dubins::ShortestPathWithoutCollisions(double x0, double y0, double th0, double xf, double yf, double thf, const std::vector<Polygon> &obstacles, const Polygon &map) {
        ...
        for (int i = 0; i < TOTAL_POSSIBLE_CURVES; i++) {
            ...
            auto *tmpCurve = new Curve(x0, y0, th0, tmpCurveResult->s1, tmpCurveResult->s2, tmpCurveResult->s3, ksigns[i][0] * k_max, ksigns[i][1] * k_max, ksigns[i][2] * k_max);
            ...
            if (curve_segments->ok && current_L < best_L) {
                ...
                for (auto &obstaclesLine: obstaclesLines) {
                    // Check if the first arc intersects with the obstacle z.
                    areThereCollisions = areThereCollisions || arcLineIntersection(tmpCurve->a1, DubinsPoint(obstaclesLine[0].x, obstaclesLine[0].y), DubinsPoint(obstaclesLine[1].x, obstaclesLine[1].y), polTmp, tTmp);
                    ...
                    if (isThereAStraightLine)
                        areThereCollisions = areThereCollisions || lineLineIntersection(DubinsPoint(tmpCurve->a2->x0, tmpCurve->a2->y0), DubinsPoint(tmpCurve->a2->dubins_line->x, tmpCurve->a2->dubins_line->y),
                                                                                        DubinsPoint(obstaclesLine[0].x, obstaclesLine[0].y), DubinsPoint(obstaclesLine[1].x, obstaclesLine[1].y), polTmp, tTmp);
                    else
                        areThereCollisions = areThereCollisions || arcLineIntersection(tmpCurve->a2, DubinsPoint(obstaclesLine[0].x, obstaclesLine[0].y), DubinsPoint(obstaclesLine[1].x, obstaclesLine[1].y), polTmp, tTmp);
                    // Check if the third arc intersects with the obstacle z.
                    areThereCollisions = areThereCollisions || arcLineIntersection(tmpCurve->a3, DubinsPoint(obstaclesLine[0].x, obstaclesLine[0].y), DubinsPoint(obstaclesLine[1].x, obstaclesLine[1].y), polTmp, tTmp);
                }
    ...
    Curve **Dubins::multipointShortestPath(DubinsPoint **points, int numberOfPoints, const std::vector<Polygon> &obstacles, const Polygon &map) {
        if (numberOfPoints > 1) {
            auto **newPoints = new DubinsPoint *[numberOfPoints];
            for (int i = 0; i < numberOfPoints; i++) {
            ...
            auto **curves = new Curve *[numberOfPoints - 1];
            // For each point
